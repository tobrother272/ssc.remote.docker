...
public class ProfileManager {
    ...
    public static String makeProfile(String username, String profilePath) {
        try {
            String[] cmd_array = {firefoxPath, "-CreateProfile", "\"" + username + " " + profilePath + "\""};
    ...
    public static String openFireFox(String profilePath, String startPage, String useragent) {
        try {
            if (startPage.length() == 0) {
                String[] cmd_array = {firefoxPath, "-profile", "\"" + profilePath + "\""};
                runProgressOpen(cmd_array);
            } else {
                String[] cmd_array = {firefoxPath, "-profile", "\"" + profilePath + "\"", "-url", "\"" + startPage + "\""};
                runProgressOpen(cmd_array);
            }
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    public static String openFireFoxNewTab(String profilePath, String startPage, String useragent) {
        try {
            if (startPage.length() == 0) {
                ...
                String[] cmd_array = {firefoxPath, "-profile", "\"" + profilePath + "\"", "-new-tab", "\"" + startPage + "\""};
                runProgressOpen(cmd_array);
            }
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    public static String openFireFox() {
        try {
            ...
            String[] cmd_array = {firefoxPath, "\"https://youtube.com/\""};
            runProgress(cmd_array);
        } catch (Exception ex) {
            ex.printStackTrace();
            return "Send text lỗi exception";
    ...
    public static String openFireFoxWithSearch(String profilePath, String searchQuery, String useragent) {
        try {
            ...
            if (searchQuery.length() == 0) {
                ...
                String[] cmd_array = {firefoxPath, "-profile", "\"" + profilePath + "\"", "-search", "\"" + searchQuery + "\""};
                runProgress(cmd_array);
            }
        } catch (Exception ex) {
            ex.printStackTrace();
